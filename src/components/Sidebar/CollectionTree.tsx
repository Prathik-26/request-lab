...
function RequestItem({ req }: { req: ApiRequest }) {
  const [renaming, setRenaming] = useState(false);
  const [renameValue, setRenameValue] = useState(req.name);
  const { saveRequest, deleteRequest } = useCollectionStore();
  const { openTab, setActive, closeTab } = useTabStore();
  ...
  const handleRename = async () => {
    if (renameValue.trim() && renameValue !== req.name) {
      const updated = {
        ...req,
  ...
  const handleDelete = async () => {
    await closeTab(req.id);
    await deleteRequest(req.id);
  };

  return (
    <div className="flex items-center gap-1 group rounded-md px-1 py-1 hover:bg-zinc-800 cursor-pointer">
      <span
      ...
      {renaming ? (
        <Input
          ...
          className="h-5 text-xs px-1 py-0 bg-zinc-700 border-zinc-600 flex-1"
        />
      ) : (
        <span
          onClick={handleOpen}
